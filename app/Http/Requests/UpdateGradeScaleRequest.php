...
class UpdateGradeScaleRequest extends FormRequest
{
    ...
    {
        return [
            'grade' => ['required', 'string', 'max:5', Rule::unique('grade_scales', 'grade')->ignore($this->grade_scale)],
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            // Check for overlapping marks ranges (excluding current grade scale)
            if ($this->min_marks !== null && $this->max_marks !== null) {
                $overlaps = GradeScale::where('id', '!=', $this->grade_scale->id)
